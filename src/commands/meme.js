    ...
    commands: [
        {
            ...
            method: async function (message, args, bot) {
                ...
                if(memeData[memeToUse]) {
                    ...
                    const canvas = Canvas.createCanvas(memeToUseData.width, memeToUseData.height);
                    const ctx = canvas.getContext('2d');
                    ctx.drawImage(memeBackground, 0, 0, canvas.width, canvas.height);
                    ctx.fillStyle = 'white';
                    ctx.textAlign = "center";
                    ctx.font = `${memeToUseData.textSize}px sans-serif`;
                    const isHorizontal = (memeToUseData.textAreas.length >= 2 && memeToUseData.textAreas[0][1] == memeToUseData.textAreas[1][1]);
