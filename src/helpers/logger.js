...
class Logger {
    static logMessage(logLevel, message) {
        ...
        if (typeof Logger.logStream === "undefined") {
            ...
            Logger.logStream = fs.createWriteStream(logPath + logFileName, {
                flag: "a",
            });
            // remove extra/unneeded log files
            Logger.logStream.on("open", () => {
                // Wait until the file is actually opened to count how many files are in directory to avoid race conditions
                fs.readdir(logPath, (err, files) => {
                    ...
                    while (dataFiles.length > config.maxLogFiles) {
                        ...
                        fs.unlink(logPath + toRemove.name, (err) => {
                            if (err) {
                                console.log(
                                    "Error when attempting to remove Log File " +
                    toRemove.name +
                    ": " +
                    err,
                                );
        ...
        if (Logger.rankLogLevel(logLevel) >= Logger.rankLogLevel(config.logLevel)) {
            const logString =
        "| [" + new Date() + "] " + logLevel.toUpperCase() + " |: " + message;
